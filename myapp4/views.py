...
def add_user(request):
    # Инициализация сообщения и формы
    message = 'Заполните форму'
    form = UserForm()

    if request.method == 'POST':
        # Создание формы из POST-данных
        form = UserForm(request.POST)
        if form.is_valid():
            # Если форма валидна, сохраняем данные пользователя
            name = form.cleaned_data['name']
            email = form.cleaned_data['email']
            age = form.cleaned_data['age']
            logger.info(f'Получили {name=}, {email=}, {age=}.')
            ...
            # Если данные невалидны, сообщаем об ошибке
            message = 'Ошибка в данных'

    # Возвращаем HttpResponse через render
    return render(request, 'myapp4/user_form.html', {'form': form, 'message': message})
...
def upload_image(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'myapp4/upload_image.html', {
                'form': form,
